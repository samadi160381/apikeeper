...
export async function checkRateLimit(keyId, limitPerMinute) {
  try {
    ...
    // Redis unreachable — fail open so the API stays usable without rate limiting
    console.error('[rateLimiter] Redis unavailable, allowing request:', err.message);
    return { allowed: true, remaining: limitPerMinute, limit: limitPerMinute, retryAfterMs: 0 };
  }
}
